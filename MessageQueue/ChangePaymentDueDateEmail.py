...
if datetime.datetime.now().weekday() == 0:
    log_filename = datetime.datetime.now().strftime("%Y_%m_%d") + "_ChangePaymentDueDateEmail.log"
else:
    monday = datetime.datetime.now() - timedelta(days = datetime.datetime.now().weekday())
    log_filename = monday.strftime("%Y_%m_%d") + "_ChangePaymentDueDateEmail.log"
...
def sendEmail():
    try:
        ...
        getMessages = MessagingQueue.objects.all().filter(status=False, module='ChangePaymentDueDateView', void=False)
        if getMessages:
                            ...
                            if message.retry != settings.RETRY_LIMIT:
                                try:
                                    ...
                                    emailContext = {
                                        ...
                                        'pdd': paymentMode.new_payment_due_date
                                    }
                                    htmlTemplate = render_to_string('EmailTemplate/CompanyPDDChangeUpdateEmail.html', emailContext)
                                    
                                    data = {
                                        'email_body'    :"",
                                        'to_email'      : getCompanyObj.email_address,
                                        'email_subject' :'Updating Payment Due Date'
                                    }
