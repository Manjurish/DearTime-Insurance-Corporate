...
if datetime.datetime.now().weekday() == 0:
    log_filename = datetime.datetime.now().strftime("%Y_%m_%d") + "_CompanyApproveEmail.log"
else:
    monday = datetime.datetime.now() - datetime.timedelta(days = datetime.datetime.now().weekday())
    log_filename = monday.strftime("%Y_%m_%d") + "_CompanyApproveEmail.log"
...
def sendEmail():
    try:
        ...
        getMessages = MessagingQueue.objects.all().filter(status=False, module='CompanyApproveEmailView', void=False)
        if getMessages:
                        ...
                        if is_multiple_exists <= 1:
                            ...
                            if message.retry != settings.RETRY_LIMIT:
                                try:
                                    ...
                                    htmlTemplate = render_to_string('EmailTemplate/CompanyApproveEmail.html', context)
