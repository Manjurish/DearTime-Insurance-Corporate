...
if datetime.datetime.now().weekday() == 0:
    log_filename = datetime.datetime.now().strftime("%Y_%m_%d") + "_CompanyDocSubmittedEmail.log"
else:
    monday = datetime.datetime.now() - datetime.timedelta(days = datetime.datetime.now().weekday())
    log_filename = monday.strftime("%Y_%m_%d") + "_CompanyDocSubmittedEmail.log"
...
def sendEmail():
    try:
        ...
        getMessages = MessagingQueue.objects.all().filter(status=False, module='CompanyRegistrationView', void=False)
        if getMessages:
                        ...
                        if is_multiple_exists <= 1:
                            ...
                            if message.retry != settings.RETRY_LIMIT:
                                try:
                                    ...
                                    site_name    = settings.HOST_ADDRESS
                                    ...
                                    htmlTemplate = render_to_string('EmailTemplate/CompanyDocSubmitted.html', context)
