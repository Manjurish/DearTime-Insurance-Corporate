...
from Portal.models import MessagingQueue, CorporateUser
...
if datetime.datetime.now().weekday() == 0:
    log_filename = datetime.datetime.now().strftime("%Y_%m_%d") + "_SignUpValidationEmail.log"
else:
    monday = datetime.datetime.now() - datetime.timedelta(days = datetime.datetime.now().weekday())
    log_filename = monday.strftime("%Y_%m_%d") + "_SignUpValidationEmail.log"
...
def sendEmail():
    try:
        ...
        getMessages = MessagingQueue.objects.all().filter(status=False, module='SignUpValidationEmailView', void=False)
        if getMessages:
                        ...
                        if is_multiple_exists <= 1:
                            ...
                            if message.retry != settings.RETRY_LIMIT:
                                try:
                                    ...
                                    relativelink = reverse('corporateportal:company_registration_page', kwargs={'uidb64': uidb64, 'token': token})
                                    absurl       = settings.HOST_ADDRESS + relativelink
                                    
                                    context      = {
                                        'Url'       : absurl
                                    }
                                    htmlTemplate = render_to_string('EmailTemplate/SignUpValidationEmail.html', context)
